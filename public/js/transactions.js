...
async function setValuesOrigin(id) {
  ...
  const data = await fetch(
    baseUrl + "/presupuestos/obtenerPresupuesto?id_presupuesto=" + id
  );
  const budgets = await data.json();
  console.log(budgets.MONTO_ACTUAL);
...
form.addEventListener("submit", function(e) {
  ...
  if (
    Number(formData.get("origin")) >= 1 &&
    Number(formData.get("destiny")) >= 1
  ) {
    validationsValues(
      Number(formData.get("origin")),
      Number(formData.get("destiny")),
    ...
    $mensajeRespuesta.innerHTML = 'Solicitud enviada';
        //console.clear();
      } else {
      }
    });
...
async function validationsValues(idOrigin, idDestiny, amount) {
  const data0 = await fetch(
    baseUrl + "/presupuestos/obtenerPresupuesto?id_presupuesto=" + idOrigin
  );
  const data1 = await fetch(
    baseUrl + "/presupuestos/obtenerPresupuesto?id_presupuesto=" + idDestiny
  );
...
function selectorValidation(idOrigin, idDestiny, amount) {
  ...
  } else if (idOrigin.ID_PRESUPUESTO == idDestiny.ID_PRESUPUESTO) {
...
function recalc(origin, destiny, amount) {
  ...
  var newOrigin = {
    id: Number(origin.ID_PRESUPUESTO),
    monto: Number(origin.MONTO_ACTUAL) - amount
  };
  ...
  var newDestiny = {
    id: Number(destiny.ID_PRESUPUESTO),
    monto: Number(destiny.MONTO_ACTUAL) + amount
  };
